(() => {
  ...
  function screen(impulse, queueCopy) {
    ...
    if (!parentImpulse)
      return false;
    if (queueCopy.has(parentImpulse))
      return true;
  ...
  function activate(callback, state, parentView, i, dom = {}, hydrateNodes) {
    ...
    const childImpulses = [];
    const view = i === void 0 ? parentView : parentView[i + 1] || [];
    const sibling = { ...dom };
    let params = [];
    ...
    function impulse(newState) {
      ...
      if (i < 0) {
        ...
        for (const impulse2 of view.impulse.childImpulses) {
          impulse2(outline);
        }
      }
      impulses.shift();
    ...
    if (!view.memos)
      Object.assign(view, { memos: [], teardowns: [] });
    if (view.impulse)
      unsubscribe(view.impulse);
  ...
  function populate(outlines, state, view, dom, hydrateNodes) {
    ...
    if (validEntries.length !== entries.length)
      view.keyedViews = Object.fromEntries(keyedEntries);
  }
  ...
  function update(node, attributes, updater, defaultProps, ignoreRef) {
  ...
  function process(outline, state, parentView, i, dom, hydrateNodes) {
    ...
    if (hydrateNodes)
      ...
      view = parentView.keyedViews[key] || view || [];
    let [node] = view;
    if (tagName === "") {
  ...
  function createElement(tagName, attributes, layout, framework = defaultFramework) {
    const [document, updater] = framework;
    const container = document.createElement(tagName);
    if (layout !== void 0)
      stew(container, layout);
    if (attributes)
      updater(container, attributes);
    return container;
  }
  Object.assign(stew, {
    useMemo,
